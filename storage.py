...
class Storage:
    def __init__(self, db_path: Path = DB_PATH):
        self.db_path = Path(db_path); self.db_path.parent.mkdir(parents=True, exist_ok=True); self._init_db(); self._purge_stale_signals()

    def connect(self):
        conn=sqlite3.connect(self.db_path,timeout=20); conn.row_factory=sqlite3.Row; return conn
    ...
    def _init_db(self):
        with self.connect() as conn:
            conn.executescript("""
                ...
                CREATE INDEX IF NOT EXISTS idx_events_start ON events(start_date); CREATE INDEX IF NOT EXISTS idx_events_title ON events(title_key);
                ...
                CREATE INDEX IF NOT EXISTS idx_social_last_seen ON social_captures(last_seen DESC); CREATE INDEX IF NOT EXISTS idx_social_status ON social_captures(status);
                ...
                CREATE TABLE IF NOT EXISTS event_feedback (event_id TEXT PRIMARY KEY,rating TEXT NOT NULL,event_snapshot TEXT NOT NULL DEFAULT '{}',updated_at TEXT NOT NULL);
                CREATE INDEX IF NOT EXISTS idx_feedback_rating ON event_feedback(rating);
            """)

    @staticmethod
    def _signal_is_current(signal:dict)->bool:
        current_year=datetime.now().astimezone().year; text=f"{signal.get('title','')} {signal.get('excerpt','')}"; years=[int(y) for y in re.findall(r"\b(20\d{2})\b",text)]; return not years or max(years)>=current_year

    def _purge_stale_signals(self):
        cutoff=(datetime.now(timezone.utc)-timedelta(days=14)).isoformat()
        with self.connect() as conn:
            rows=conn.execute("SELECT id,title,excerpt,last_seen FROM signals").fetchall(); stale=[r['id'] for r in rows if r['last_seen']<cutoff or not self._signal_is_current(dict(r))]
    ...
    @staticmethod
    def title_key(title:str)->str: return " ".join("".join(ch.lower() if ch.isalnum() else " " for ch in title).split())
    ...
    def list_events(self):
        ...
        with self.connect() as conn: rows=conn.execute("SELECT payload FROM events WHERE start_date>=? ORDER BY start_date,title_key",(today,)).fetchall(); feedback={r['event_id']:r['rating'] for r in conn.execute("SELECT event_id,rating FROM event_feedback").fetchall()}
        out=[]
        for row in rows:
            event=json.loads(row['payload']); event['feedback']=feedback.get(event.get('id')); out.append(event)
        return out

    def set_feedback(self,event_id:str,rating:str,snapshot:dict|None=None):
        if rating not in {'interested','not_for_me','loved'}: return False
        with self.connect() as conn: conn.execute("INSERT INTO event_feedback(event_id,rating,event_snapshot,updated_at) VALUES(?,?,?,?) ON CONFLICT(event_id) DO UPDATE SET rating=excluded.rating,event_snapshot=excluded.event_snapshot,updated_at=excluded.updated_at",(event_id,rating,json.dumps(snapshot or {},ensure_ascii=False),now_iso()))
        return True

    def clear_feedback(self,event_id:str):
        with self.connect() as conn: conn.execute("DELETE FROM event_feedback WHERE event_id=?",(event_id,))

    def preference_profile(self):
        weights={'interested':1,'loved':3,'not_for_me':-2}; categories=Counter(); terms=Counter(); counts=Counter()
        with self.connect() as conn: rows=conn.execute("SELECT rating,event_snapshot FROM event_feedback ORDER BY updated_at DESC").fetchall()
        stop={'event','free','savannah','the','and','with','for','this','that','from','your','you','are','our','will','all','day','at','in','of','to','a','an'}
        for row in rows:
            rating=row['rating']; counts[rating]+=1; w=weights.get(rating,0)
            try: e=json.loads(row['event_snapshot'] or '{}')
            except json.JSONDecodeError: e={}
            for cat in e.get('categories',[]) or []: categories[str(cat).lower()]+=w
            text=f"{e.get('title','')} {e.get('description','')}"
            for term in re.findall(r"[a-z][a-z0-9'-]{2,}",text.lower()):
                if term not in stop: terms[term]+=w
        return {'counts':dict(counts),'category_weights':dict(categories.most_common()),'positive_terms':[k for k,v in terms.most_common(20) if v>0],'negative_terms':[k for k,v in sorted(terms.items(),key=lambda x:x[1])[:20] if v<0]}

    def upsert_signal(self,signal:dict):
        if not self._signal_is_current(signal): return False
        with self.connect() as conn:
            exists=conn.execute("SELECT id FROM signals WHERE id=?",(signal['id'],)).fetchone(); conn.execute("INSERT INTO signals(id,source_id,title,excerpt,url,kind,first_seen,last_seen) VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET last_seen=excluded.last_seen,excerpt=excluded.excerpt,title=excluded.title",(signal['id'],signal['source_id'],signal['title'],signal['excerpt'],signal['url'],signal.get('kind','special-interest'),signal.get('first_seen',now_iso()),now_iso()))
    ...
    def mark_social_analysis(self,capture_id:str,status:str,result:dict,event_id=None):
        ts=now_iso()
        with self.connect() as conn:
            conn.execute("UPDATE social_captures SET status=? WHERE id=?",(status,capture_id)); conn.execute("INSERT INTO social_analysis(capture_id,status,result_json,event_id,updated_at) VALUES(?,?,?,?,?) ON CONFLICT(capture_id) DO UPDATE SET status=excluded.status,result_json=excluded.result_json,event_id=excluded.event_id,updated_at=excluded.updated_at",(capture_id,status,json.dumps(result,ensure_ascii=False),event_id,ts))
    ...
    def social_stats(self):
        with self.connect() as conn: total=conn.execute("SELECT COUNT(*) AS n FROM social_captures").fetchone()['n']; counts={row['status']:row['n'] for row in conn.execute("SELECT status,COUNT(*) AS n FROM social_captures GROUP BY status").fetchall()}; last=conn.execute("SELECT max(last_seen) AS ts FROM social_captures").fetchone()['ts']
    ...
    def stats(self):
        with self.connect() as conn: events=conn.execute("SELECT COUNT(*) AS n FROM events").fetchone()['n']; signals=conn.execute("SELECT COUNT(*) AS n FROM signals").fetchone()['n']; checked=conn.execute("SELECT COUNT(*) AS n FROM source_status WHERE last_checked IS NOT NULL").fetchone()['n']; feedback=conn.execute("SELECT COUNT(*) AS n FROM event_feedback").fetchone()['n']
        return {'events':events,'signals':signals,'sources_checked':checked,'feedback':feedback,'db_path':str(self.db_path),'social':self.social_stats()}
